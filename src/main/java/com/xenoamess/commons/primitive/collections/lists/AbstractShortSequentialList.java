...
public abstract class AbstractShortSequentialList extends AbstractShortList implements Primitive {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected AbstractShortSequentialList() {
    }
    ...
    public short setPrimitive(int index, short element) {
        try {
            ShortListIterator e = listIterator(index);
            short oldVal = e.nextPrimitive();
            e.set(element);
            return oldVal;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: " + index);
    ...
    public boolean addAll(int index, Collection<? extends Short> c) {
        try {
            ...
            ShortListIterator e1 = listIterator(index);
            if (c instanceof ShortCollection) {
                ...
                while (cShortCollectionIterator.hasNext()) {
                    e1.addPrimitive(cShortCollectionIterator.nextPrimitive());
                    modified = true;
                }
            } else {
                for (Short e : c) {
