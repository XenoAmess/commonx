...
////     */
////    @Override
////    default boolean remove(short o) {
...
////     */
////    @Override
////    boolean removePrimitive(short o);
...
//    boolean retainAll(Collection<?> c);
    ...
    default void replaceAll(UnaryOperator<Short> operator) {
        ...
        final ShortListIterator li = this.listIterator();
        if (operator instanceof ShortUnaryOperator) {
            ShortUnaryOperator operatorShortUnaryOperator = (ShortUnaryOperator) operator;
            while (li.hasNext()) {
                li.setPrimitive(operatorShortUnaryOperator.applyPrimitive(li.nextPrimitive()));
            }
        } else {
            while (li.hasNext()) {
                li.set(operator.apply(li.next()));
    ...
    default void sort(Comparator<? super Short> c) {
        if (c instanceof ShortComparator) {
            short[] a = this.toArrayPrimitive();
            Arraysx.sort(a, 0, a.length, (ShortComparator) c);
     ...
     * @throws java.lang.IndexOutOfBoundsException     if the index is out of range
     *                                                 ({@code index < 0 || index >= size()})
     * @see #set(int index, Short element)
     */
    default short set(int index, short element) {
        return this.setPrimitive(index, element);
    }
     ...
     * @see #set(int index, Short element)
     */
    short setPrimitive(int index, short element);
     ...
     * @see #add(int index, Short element)
     */
    void addPrimitive(int index, short element);
     ...
     */
    short removeByIndexPrimitive(int index);
     ...
     */
    int lastIndexOfPrimitive(short o);
    ...
    default ShortSpliterator spliterator() {
        if (this instanceof RandomAccess) {
            return new ShortRandomAccessSpliterator(this);
        } else {
            return ShortSpliterators.spliterator(this, Spliterator.ORDERED);
        }
    }
