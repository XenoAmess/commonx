...
public abstract class AbstractDoubleSequentialList extends AbstractDoubleList implements Primitive {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected AbstractDoubleSequentialList() {
    }
    ...
    public double setPrimitive(int index, double element) {
        try {
            DoubleListIterator e = listIterator(index);
            double oldVal = e.nextPrimitive();
            e.set(element);
            return oldVal;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: " + index);
    ...
    public double removePrimitive(int index) {
        try {
            DoubleListIterator e = listIterator(index);
            double outCast = e.nextPrimitive();
            e.remove();
            return outCast;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: " + index);
    ...
    public boolean addAll(int index, Collection<? extends Double> c) {
        try {
            ...
            DoubleListIterator e1 = listIterator(index);
            if (c instanceof DoubleCollection) {
                ...
                while (cDoubleCollectionIterator.hasNext()) {
                    e1.addPrimitive(cDoubleCollectionIterator.nextPrimitive());
                    modified = true;
                }
            } else {
                for (Double e : c) {
