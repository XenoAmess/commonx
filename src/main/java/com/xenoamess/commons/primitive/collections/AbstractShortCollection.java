 ...
 * @since 1.2
 */
public interface AbstractShortCollection extends ShortCollection,
        Primitive {
     ...
     */
    @Override
    default boolean containsPrimitive(short o) {
    ...
    default short[] toArrayPrimitive(short[] a) {
        // Estimate size of array; be prepared to see more or fewer elements
        int size = size();
        short[] r = a.length >= size ? a : new short[size];
        ShortIterator it = iterator();

        for (int i = 0; i < r.length; i++) {
            if (!it.hasNext()) { // fewer elements than expected
                if (a == r) {
                    r[i] = Primitive.SHORT_DEFAULT; // null-terminate
                } else if (a.length < i) {
                    return Arrays.copyOf(r, i);
                } else {
                    System.arraycopy(r, 0, a, 0, i);
                    if (a.length > i) {
                        a[i] = Primitive.SHORT_DEFAULT;
                    }
                }
                return a;
            }
    ...
    default boolean remove(Object o) {
        ...
        return this.removeByContentPrimitive((Short) o);
    }
     ...
     */
    @Override
    default boolean removeByContentPrimitive(short o) {
        ShortIterator it = iterator();
        while (it.hasNext()) {
            if (o == it.nextPrimitive()) {
                it.remove();
    ...
    default boolean containsAll(Collection<?> c) {
        if (c instanceof ShortCollection) {
            ...
            while (cShortCollectionIterator.hasNext()) {
                if (!containsPrimitive(cShortCollectionIterator.nextPrimitive())) {
                    return false;
                }
            }
        } else {
     ...
     */
    @Override
    default boolean addAll(Collection<? extends Short> c) {
        boolean modified = false;
        if (c instanceof ShortCollection) {
            ShortCollection cShortCollection = (ShortCollection) c;
            ShortIterator cShortCollectionIterator = cShortCollection.iterator();
            while (cShortCollectionIterator.hasNext()) {
                if (add(cShortCollectionIterator.nextPrimitive())) {
                    modified = true;
                }
            }
        } else {
    ...
    default boolean removeAll(Collection<?> c) {
        ...
        if (c instanceof ShortCollection) {
            ShortCollection cShortCollection = (ShortCollection) c;
            while (it.hasNext()) {
                if (cShortCollection.containsPrimitive(it.nextPrimitive())) {
                    it.remove();
                    modified = true;
                }
            }
     ...
     *
     * @param abstractShortCollection a {@link com.xenoamess.commons.primitive.collections.AbstractShortCollection}
     *                                object.
     * @return a string representation of this collection
     */
    static String toString(AbstractShortCollection abstractShortCollection) {
        ShortIterator it = abstractShortCollection.iterator();
        if (!it.hasNext()) {
            return "[]";
        }
