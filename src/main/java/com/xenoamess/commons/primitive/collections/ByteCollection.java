...
public interface ByteCollection extends Collection<Byte>, ByteIterable, Primitive {
    ...
    default boolean contains(Object o) {
        ...
        return this.containsPrimitive((Byte) o);
    }
     ...
     */
    default boolean contains(byte o) {
     ...
     * @see #contains(Object o)
     */
    boolean containsPrimitive(byte o);
    ...
    /**
     * Returns an byte array containing all of the elements in this byte collection.
     * If this byte collection makes any guarantees as to what order its elements
     * are returned by its iterator, this method must return the elements in
     * the same order. The returned array's {@linkplain Class#getComponentType
     * runtime component type} is {@code byte}.
     *
     ...
     * the array.
     *
     * Suppose {@code x} is a byte collection.
     ...
     * @apiNote This method acts as a bridge between array-based and collection-based APIs.
     * It allows creation of an array of a particular runtime type. Use
     * {@link #toArrayPrimitive()} to create an array whose runtime type is {@code byte[]},
     * or use {@link #toArrayPrimitive(byte[])} to reuse an existing array.
     *
     * Suppose {@code x} is a collection known to contain only strings.
     * The following code can be used to dump the collection into a newly
     * allocated array of {@code byte}:
     *
     * <pre>
     *     byte[] y = x.toArrayPrimitive(byte[]::new);</pre>
     * @implSpec The default implementation calls the generator function with zero
     * and then passes the resulting array to {@link #toArrayPrimitive(byte[])}.
     * @since 11
     */
    default byte[] toArrayPrimitive(IntFunction<byte[]> generator) {
     ...
     * @see #add(Byte e)
     */
    default boolean addPrimitive(byte e) {
    ...
    default boolean remove(Object o) {
        ...
        return removeByContentPrimitive((Byte) o);
    }
     ...
     * @see #remove(Object o)
     */
    default boolean removeByContent(byte o) {
     ...
     * @see #remove(Object o)
     */
    boolean removeByContentPrimitive(byte o);
    ...
    default boolean removeIf(Predicate<? super Byte> filter) {
        ...
        if (filter instanceof BytePredicate) {
            BytePredicate actionBytePredicate = (BytePredicate) filter;
            while (each.hasNext()) {
                if (actionBytePredicate.testPrimitive(each.nextPrimitive())) {
                    each.remove();
                    removed = true;
                }
            }
