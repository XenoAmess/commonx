 ...
 * @since 1.2
 */
public interface AbstractLongCollection extends LongCollection,
        Primitive {
     ...
     */
    @Override
    default boolean containsPrimitive(long o) {
     ...
     * further elements returned by the iterator, trimmed to size
     */
    static long[] finishToArrayPrimitive(long[] r, LongIterator it) {
        int i = r.length;
        while (it.hasNext()) {
            int cap = r.length;
            if (i == cap) {
     ...
     */
    @Override
    default boolean removeByContentPrimitive(long o) {
        LongIterator it = iterator();
        while (it.hasNext()) {
            if (o == it.nextPrimitive()) {
                it.remove();
    ...
    default boolean containsAll(Collection<?> c) {
        if (c instanceof LongCollection) {
            ...
            while (cLongCollectionIterator.hasNext()) {
                if (!containsPrimitive(cLongCollectionIterator.nextPrimitive())) {
                    return false;
                }
            }
        } else {
    ...
    default boolean addAll(Collection<? extends Long> c) {
        ...
        if (c instanceof LongCollection) {
            ...
            LongIterator cLongCollectionIterator = cLongCollection.iterator();
            while (cLongCollectionIterator.hasNext()) {
                if (add(cLongCollectionIterator.nextPrimitive())) {
                    modified = true;
                }
            }
        } else {
    ...
    default boolean removeAll(Collection<?> c) {
        ...
        if (c instanceof LongCollection) {
            LongCollection cLongCollection = (LongCollection) c;
            while (it.hasNext()) {
                if (cLongCollection.containsPrimitive(it.nextPrimitive())) {
                    it.remove();
                    modified = true;
                }
            }
     ...
     *
     * @param abstractLongCollection a {@link com.xenoamess.commons.primitive.collections.AbstractLongCollection}
     *                               object.
     * @return a string representation of this collection
     */
    static String toString(AbstractLongCollection abstractLongCollection) {
        LongIterator it = abstractLongCollection.iterator();
        if (!it.hasNext()) {
            return "[]";
        }
