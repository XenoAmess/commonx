...
public abstract class AbstractLongSequentialList extends AbstractLongList implements Primitive {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected AbstractLongSequentialList() {
    }
    ...
    public long setPrimitive(int index, long element) {
        try {
            LongListIterator e = listIterator(index);
            long oldVal = e.nextPrimitive();
            e.set(element);
            return oldVal;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: " + index);
    ...
    public boolean addAll(int index, Collection<? extends Long> c) {
        try {
            ...
            LongListIterator e1 = listIterator(index);
            if (c instanceof LongCollection) {
                ...
                while (cLongCollectionIterator.hasNext()) {
                    e1.addPrimitive(cLongCollectionIterator.nextPrimitive());
                    modified = true;
                }
            } else {
                for (Long e : c) {
