 ...
 * @since 1.2
 */
public interface AbstractDoubleCollection extends DoubleCollection,
        Primitive {
     ...
     */
    @Override
    default boolean containsPrimitive(double o) {
    ...
    default double[] toArrayPrimitive(double[] a) {
        ...
        for (int i = 0; i < r.length; i++) {
            if (!it.hasNext()) { // fewer elements than expected
                if (a == r) {
                    r[i] = Primitive.DOUBLE_DEFAULT; // null-terminate
                } else if (a.length < i) {
                    return Arrays.copyOf(r, i);
                } else {
                    System.arraycopy(r, 0, a, 0, i);
                    if (a.length > i) {
                        a[i] = Primitive.DOUBLE_DEFAULT;
                    }
                }
                return a;
            }
     ...
     * further elements returned by the iterator, trimmed to size
     */
    static double[] finishToArrayPrimitive(double[] r, DoubleIterator it) {
        int i = r.length;
        while (it.hasNext()) {
            int cap = r.length;
            if (i == cap) {
    ...
    default boolean containsAll(Collection<?> c) {
        if (c instanceof DoubleCollection) {
            ...
            while (cDoubleCollectionIterator.hasNext()) {
                if (!containsPrimitive(cDoubleCollectionIterator.nextPrimitive())) {
                    return false;
                }
            }
        } else {
    ...
    default boolean addAll(Collection<? extends Double> c) {
        ...
        if (c instanceof DoubleCollection) {
            ...
            DoubleIterator cDoubleCollectionIterator = cDoubleCollection.iterator();
            while (cDoubleCollectionIterator.hasNext()) {
                if (add(cDoubleCollectionIterator.nextPrimitive())) {
                    modified = true;
                }
            }
        } else {
    ...
    default boolean removeAll(Collection<?> c) {
        ...
        if (c instanceof DoubleCollection) {
            DoubleCollection cDoubleCollection = (DoubleCollection) c;
            while (it.hasNext()) {
                if (cDoubleCollection.containsPrimitive(it.nextPrimitive())) {
                    it.remove();
                    modified = true;
                }
            }
     ...
     *
     * @param abstractDoubleCollection a {@link com.xenoamess.commons.primitive.collections.AbstractDoubleCollection}
     *                                 object.
     * @return a string representation of this collection
     */
    static String toString(AbstractDoubleCollection abstractDoubleCollection) {
        DoubleIterator it = abstractDoubleCollection.iterator();
        if (!it.hasNext()) {
            return "[]";
        }
