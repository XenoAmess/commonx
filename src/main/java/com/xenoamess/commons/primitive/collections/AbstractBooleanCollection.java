 ...
 * @since 1.2
 */
public interface AbstractBooleanCollection extends BooleanCollection,
        Primitive {
     ...
     */
    @Override
    default boolean containsPrimitive(boolean o) {
     ...
     */
    @Override
    default boolean[] toArrayPrimitive(boolean[] a) {
        // Estimate size of array; be prepared to see more or fewer elements
        int size = size();
        boolean[] r = a.length >= size ? a : new boolean[size];
        BooleanIterator it = iterator();

        for (int i = 0; i < r.length; i++) {
            if (!it.hasNext()) { // fewer elements than expected
                if (a == r) {
                    r[i] = Primitive.BOOLEAN_DEFAULT; // null-terminate
                } else if (a.length < i) {
                    return Arrays.copyOf(r, i);
                } else {
                    System.arraycopy(r, 0, a, 0, i);
                    if (a.length > i) {
                        a[i] = Primitive.BOOLEAN_DEFAULT;
                    }
                }
                return a;
            }
     ...
     * further elements returned by the iterator, trimmed to size
     */
    static boolean[] finishToArrayPrimitive(boolean[] r, BooleanIterator it) {
        int i = r.length;
        while (it.hasNext()) {
            int cap = r.length;
            if (i == cap) {
     ...
     */
    @Override
    default boolean removeByContentPrimitive(boolean o) {
        BooleanIterator it = iterator();
        while (it.hasNext()) {
            if (o == it.nextPrimitive()) {
                it.remove();
    ...
    default boolean containsAll(Collection<?> c) {
        if (c instanceof BooleanCollection) {
            ...
            while (cBooleanCollectionIterator.hasNext()) {
                if (!containsPrimitive(cBooleanCollectionIterator.nextPrimitive())) {
                    return false;
                }
            }
        } else {
     ...
     */
    @Override
    default boolean addAll(Collection<? extends Boolean> c) {
        boolean modified = false;
        if (c instanceof BooleanCollection) {
            BooleanCollection cBooleanCollection = (BooleanCollection) c;
            BooleanIterator cBooleanCollectionIterator = cBooleanCollection.iterator();
            while (cBooleanCollectionIterator.hasNext()) {
                if (add(cBooleanCollectionIterator.nextPrimitive())) {
                    modified = true;
                }
            }
        } else {
    ...
    default boolean removeAll(Collection<?> c) {
        ...
        if (c instanceof BooleanCollection) {
            BooleanCollection cBooleanCollection = (BooleanCollection) c;
            while (it.hasNext()) {
                if (cBooleanCollection.containsPrimitive(it.nextPrimitive())) {
                    it.remove();
                    modified = true;
                }
            }
     ...
     *
     * @param abstractBooleanCollection a {@link com.xenoamess.commons.primitive.collections.AbstractBooleanCollection}
     *                                  object.
     * @return a string representation of this collection
     */
    static String toString(AbstractBooleanCollection abstractBooleanCollection) {
        BooleanIterator it = abstractBooleanCollection.iterator();
        if (!it.hasNext()) {
            return "[]";
        }
