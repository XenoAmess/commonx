...
public abstract class AbstractCharSequentialList extends AbstractCharList implements Primitive {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected AbstractCharSequentialList() {
    }
    ...
    public char setPrimitive(int index, char element) {
        try {
            CharListIterator e = listIterator(index);
            char oldVal = e.nextPrimitive();
            e.set(element);
            return oldVal;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: " + index);
    ...
    public boolean addAll(int index, Collection<? extends Character> c) {
        try {
            ...
            CharListIterator e1 = listIterator(index);
            if (c instanceof CharCollection) {
                ...
                while (cCharCollectionIterator.hasNext()) {
                    e1.addPrimitive(cCharCollectionIterator.nextPrimitive());
                    modified = true;
                }
            } else {
                for (Character e : c) {
