 ...
 * @since 1.2
 */
public interface AbstractByteCollection extends ByteCollection,
        Primitive {
     ...
     */
    @Override
    default boolean containsPrimitive(byte o) {
    ...
    default byte[] toArrayPrimitive(byte[] a) {
        // Estimate size of array; be prepared to see more or fewer elements
        int size = size();
        byte[] r = a.length >= size ? a : new byte[size];
        ByteIterator it = iterator();
     ...
     */
    @SuppressWarnings("unchecked")
    static byte[] finishToArrayPrimitive(byte[] r, ByteIterator it) {
        int i = r.length;
        while (it.hasNext()) {
            int cap = r.length;
            if (i == cap) {
     ...
     */
    @Override
    default boolean removeByContentPrimitive(byte o) {
        ByteIterator it = iterator();
        while (it.hasNext()) {
            if (o == it.nextPrimitive()) {
                it.remove();
    ...
    default boolean containsAll(Collection<?> c) {
        if (c instanceof ByteCollection) {
            ...
            while (cByteCollectionIterator.hasNext()) {
                if (!containsPrimitive(cByteCollectionIterator.nextPrimitive())) {
                    return false;
                }
            }
        } else {
    ...
    default boolean addAll(Collection<? extends Byte> c) {
        ...
        if (c instanceof ByteCollection) {
            ...
            ByteIterator cByteCollectionIterator = cByteCollection.iterator();
            while (cByteCollectionIterator.hasNext()) {
                if (add(cByteCollectionIterator.nextPrimitive())) {
                    modified = true;
                }
            }
        } else {
    ...
    default boolean removeAll(Collection<?> c) {
        ...
        if (c instanceof ByteCollection) {
            ByteCollection cByteCollection = (ByteCollection) c;
            while (it.hasNext()) {
                if (cByteCollection.containsPrimitive(it.nextPrimitive())) {
                    it.remove();
                    modified = true;
                }
            }
     ...
     *
     * @param abstractByteCollection a {@link com.xenoamess.commons.primitive.collections.AbstractByteCollection}
     *                               object.
     * @return a string representation of this collection
     */
    static String toString(AbstractByteCollection abstractByteCollection) {
        ByteIterator it = abstractByteCollection.iterator();
        if (!it.hasNext()) {
            return "[]";
        }
