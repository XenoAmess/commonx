 ...
 * for some implementations (the {@code FloatLinkedList} class, for
 ...
 * @see AbstractList
 * @see AbstractFloatSequentialList
 * @see List
 * @since 1.2
 */
...
////     */
////    @Override
////    default boolean remove(float o) {
...
////     */
////    @Override
////    boolean removePrimitive(float o);
...
//    boolean retainAll(Collection<?> c);
    ...
    default void replaceAll(UnaryOperator<Float> operator) {
        ...
        if (operator instanceof FloatUnaryOperator) {
            FloatUnaryOperator operatorFloatUnaryOperator = (FloatUnaryOperator) operator;
            while (li.hasNext()) {
                li.setPrimitive(operatorFloatUnaryOperator.applyPrimitive(li.nextPrimitive()));
            }
        } else {
            while (li.hasNext()) {
                li.set(operator.apply(li.next()));
    ...
    default void sort(Comparator<? super Float> c) {
        if (c instanceof FloatComparator) {
            float[] a = this.toArrayPrimitive();
            Arraysx.sort(a, 0, a.length, (FloatComparator) c);
     ...
     * @throws java.lang.IndexOutOfBoundsException     if the index is out of range
     *                                                 ({@code index < 0 || index >= size()})
     * @see #set(int index, Float element)
     */
    default float set(int index, float element) {
        return this.setPrimitive(index, element);
    }
     ...
     * @see #add(int index, Float element)
     */
    void addPrimitive(int index, float element);
     ...
     */
    float removeByIndexPrimitive(int index);
    ...
    default int indexOf(Object o) {
        ...
        return this.indexOfPrimitive((Float) o);
    }
    ...
    default int lastIndexOf(Object o) {
        ...
        return this.lastIndexOfPrimitive((Float) o);
    }
     ...
     */
    default int lastIndexOf(float o) {
     ...
     * @see #lastIndexOf(Object o)
     */
    int lastIndexOfPrimitive(float o);
    ...
    default FloatSpliterator spliterator() {
        if (this instanceof RandomAccess) {
            return new FloatRandomAccessSpliterator(this);
        } else {
            return FloatSpliterators.spliterator(this, Spliterator.ORDERED);
        }
    }
