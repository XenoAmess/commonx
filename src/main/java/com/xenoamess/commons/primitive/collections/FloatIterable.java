 ...
 * @since 1.5
 */
public interface FloatIterable extends Iterable<Float>, Primitive {
    /**
     * {@inheritDoc}
     * <p>
    ...
    default void forEach(Consumer<? super Float> action) {
        ...
        if (action instanceof FloatConsumer) {
            ...
            while (floatIterator.hasNext()) {
                actionFloatConsumer.acceptPrimitive(floatIterator.nextPrimitive());
            }
        } else {
            for (Float t : this) {
                action.accept(t);
    ...
    default FloatSpliterator spliterator() {
        return FloatSpliterators.spliteratorUnknownSize(iterator(), 0);
    }
}
