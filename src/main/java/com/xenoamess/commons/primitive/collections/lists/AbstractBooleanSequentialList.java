...
public abstract class AbstractBooleanSequentialList extends AbstractBooleanList implements Primitive {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected AbstractBooleanSequentialList() {
    }
    ...
    public boolean setPrimitive(int index, boolean element) {
        try {
            BooleanListIterator e = listIterator(index);
            boolean oldVal = e.nextPrimitive();
            e.set(element);
            return oldVal;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: " + index);
    ...
    public boolean removePrimitive(int index) {
        try {
            BooleanListIterator e = listIterator(index);
            boolean outCast = e.nextPrimitive();
            e.remove();
            return outCast;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: " + index);
    ...
    public boolean addAll(int index, Collection<? extends Boolean> c) {
        try {
            ...
            BooleanListIterator e1 = listIterator(index);
            if (c instanceof BooleanCollection) {
                ...
                while (cBooleanCollectionIterator.hasNext()) {
                    e1.addPrimitive(cBooleanCollectionIterator.nextPrimitive());
                    modified = true;
                }
            } else {
                for (Boolean e : c) {
