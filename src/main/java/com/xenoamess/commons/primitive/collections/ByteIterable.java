 ...
 * @since 1.5
 */
public interface ByteIterable extends Iterable<Byte>, Primitive {
    /**
     * {@inheritDoc}
     * <p>
    ...
    default void forEach(Consumer<? super Byte> action) {
        ...
        if (action instanceof ByteConsumer) {
            ...
            while (byteIterator.hasNext()) {
                actionByteConsumer.acceptPrimitive(byteIterator.nextPrimitive());
            }
        } else {
            for (Byte t : this) {
                action.accept(t);
    ...
    default ByteSpliterator spliterator() {
        return ByteSpliterators.spliteratorUnknownSize(iterator(), 0);
    }
}
