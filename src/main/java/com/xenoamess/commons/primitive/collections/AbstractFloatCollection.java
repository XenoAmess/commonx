 ...
 * @since 1.2
 */
public interface AbstractFloatCollection extends FloatCollection,
        Primitive {
     ...
     */
    @Override
    default boolean containsPrimitive(float o) {
     ...
     * }</pre>
     */
    default float[] toArrayPrimitive() {
        // Estimate size of array; be prepared to see more or fewer elements
    ...
    default float[] toArrayPrimitive(float[] a) {
        // Estimate size of array; be prepared to see more or fewer elements
        int size = size();
        float[] r = a.length >= size ? a : new float[size];
        FloatIterator it = iterator();

        for (int i = 0; i < r.length; i++) {
            if (!it.hasNext()) { // fewer elements than expected
                if (a == r) {
                    r[i] = Primitive.FLOAT_DEFAULT; // null-terminate
                } else if (a.length < i) {
                    return Arrays.copyOf(r, i);
                } else {
                    System.arraycopy(r, 0, a, 0, i);
                    if (a.length > i) {
                        a[i] = Primitive.FLOAT_DEFAULT;
                    }
                }
                return a;
            }
     ...
     */
    @Override
    default boolean removeByContentPrimitive(float o) {
        FloatIterator it = iterator();
        while (it.hasNext()) {
            if (o == it.nextPrimitive()) {
                it.remove();
    ...
    default boolean containsAll(Collection<?> c) {
        if (c instanceof FloatCollection) {
            ...
            while (cFloatCollectionIterator.hasNext()) {
                if (!containsPrimitive(cFloatCollectionIterator.nextPrimitive())) {
                    return false;
                }
            }
        } else {
     ...
     */
    @Override
    default boolean addAll(Collection<? extends Float> c) {
        boolean modified = false;
        if (c instanceof FloatCollection) {
            FloatCollection cFloatCollection = (FloatCollection) c;
            FloatIterator cFloatCollectionIterator = cFloatCollection.iterator();
            while (cFloatCollectionIterator.hasNext()) {
                if (add(cFloatCollectionIterator.nextPrimitive())) {
                    modified = true;
                }
            }
        } else {
    ...
    default boolean removeAll(Collection<?> c) {
        ...
        if (c instanceof FloatCollection) {
            FloatCollection cFloatCollection = (FloatCollection) c;
            while (it.hasNext()) {
                if (cFloatCollection.containsPrimitive(it.nextPrimitive())) {
                    it.remove();
                    modified = true;
                }
            }
     ...
     *
     * @param abstractFloatCollection a {@link com.xenoamess.commons.primitive.collections.AbstractFloatCollection}
     *                                object.
     * @return a string representation of this collection
     */
    static String toString(AbstractFloatCollection abstractFloatCollection) {
        FloatIterator it = abstractFloatCollection.iterator();
        if (!it.hasNext()) {
            return "[]";
        }
