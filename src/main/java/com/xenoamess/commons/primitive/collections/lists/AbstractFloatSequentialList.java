...
public abstract class AbstractFloatSequentialList extends AbstractFloatList implements Primitive {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected AbstractFloatSequentialList() {
    }
    ...
    public float setPrimitive(int index, float element) {
        try {
            FloatListIterator e = listIterator(index);
            float oldVal = e.nextPrimitive();
            e.set(element);
            return oldVal;
        } catch (NoSuchElementException exc) {
            throw new IndexOutOfBoundsException("Index: " + index);
    ...
    public boolean addAll(int index, Collection<? extends Float> c) {
        try {
            ...
            FloatListIterator e1 = listIterator(index);
            if (c instanceof FloatCollection) {
                ...
                while (cFloatCollectionIterator.hasNext()) {
                    e1.addPrimitive(cFloatCollectionIterator.nextPrimitive());
                    modified = true;
                }
            } else {
                for (Float e : c) {
